...
function saveData(response, marketArr) {
 ...
 function getDirections() {
     console.log("getDirections triggered");
     $("#map").addClass("loading");
     clearMap();
     markersArr = [];
     if(currentLocation == null) {
         console.log("currentLocation was null");
         start = getLocation(function(){
             finish = filteredMarkets[activeMarketIndex].y + "," + filteredMarkets[activeMarketIndex].x;
             calculateRoute(start, finish);
         });
     }
     else {
         console.log("currentLocation was not null");
         finish = filteredMarkets[activeMarketIndex].y + "," + filteredMarkets[activeMarketIndex].x;
         calculateRoute(currentLocation, finish);
     }
 }
...
function goToWall() {
    ...
    // $.post("/js2py", {"market": filteredMarkets[this.marketIndex], "csrfmiddlewaretoken": getCookie("csrftoken")});
}
